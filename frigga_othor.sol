...
contract FriggaThor is Ownable, Initializable{
    ...
    function initialize(address rootAddress, address techAddress) public initializer {
        ...
        shareAirdroped = 0;
        miningAirdroped = 0;
        userShareAirdrop = [uint256(0),0,0,0,0,0,0,0];
        todayMiningIncome = 0;
        totalPower = 0;
        rewardSco = [uint256(1000000000000000000000),3000000000000000000000,5000000000000000000000,7000000000000000000000,9000000000000000000000,15000000000000000000000];
        rewardScale = [uint256(1),3,5,7,9,15];
        rewardUser = [address(0),address(0),address(0),address(0),address(0),address(0)];
        unprofitUserCount = [uint256(0),0,0,0,0,0,0,0];
        shareLevelScale = [uint256(5),5,10,10,10,15,20,25];
        lastDay = today();
    ...
    function _airdropSingle(address user, uint256 day) private {
        ...
        if(putCoins[user] > getCoins[user]){
            for (uint8 i = 0; i < shareLevelScale.length; i++) {
                if(users[user].activeMatrixLevels[i+1] && unprofitUserCount[i]>0){
                    if (userShareAirdrop[i] > 0){//air drop coins
                        amount1 = amount1.add(userShareAirdrop[i]);
                        shareGetCoins[user] = shareGetCoins[user].add(userShareAirdrop[i]);
                        shareAirdroped = shareAirdroped.add(userShareAirdrop[i]);
                    }
                }
            }
        } 
